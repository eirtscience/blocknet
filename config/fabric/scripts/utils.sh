...
setOrdererGlobals() {
  ...
  CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/blackcreek.tech/orderers/orderer.blackcreek.tech/msp/tlscacerts/tlsca.blackcreek.tech-cert.pem
  CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/blackcreek.tech/users/admin@blackcreek.tech/msp
...
if [[ $ORG =~ ^[0-9]+$ ]]; then
  ...
  if [ $ORG = 'DC' ]; then
    ...
    if [ $PEER -eq 0 ]; then
      CORE_PEER_ADDRESS=peer0.dc.blackcreek.tech:7054 
    else
...
updateAnchorPeers() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer channel update -o orderer.blackcreek.tech:7050 -c $CHANNEL_NAME -f ./channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx >&log.txt
    res=$?
    set +x
  else
    set -x
    peer channel update -o orderer.blackcreek.tech:7050 -c $CHANNEL_NAME -f ./channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.blackcreek.tech:7050 -C $CHANNEL_NAME -n ${CHAINCODE_NAME} -l ${LANGUAGE} -v ${VERSION} -c '{"function":"initLedger","Args":[]}' -P "AND ('DCMSP.peer','DPMSP.peer')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode instantiate -o orderer.blackcreek.tech:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CHAINCODE_NAME} -l ${LANGUAGE} -v ${VERSION} -c '{"function":"initLedger","Args":[]}' -P "AND ('DCMSP.peer','DPMSP.peer')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
upgradeChaincode() {
  ...
  set -x
  peer chaincode upgrade -o orderer.blackcreek.tech:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CHAINCODE_NAME} -v 0.0.2 -c '{"function":"initLedger","Args":[]}' -P "AND ('DCMSP.peer','DPMSP.peer')"
  res=$?
  set +x
  cat log.txt
...
fetchChannelConfig() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer channel fetch config config_block.pb -o orderer.blackcreek.tech:7050 -c $CHANNEL --cafile $ORDERER_CA
    set +x
  else
    set -x
    peer channel fetch config config_block.pb -o orderer.blackcreek.tech:7050 -c $CHANNEL --tls --cafile $ORDERER_CA
    set +x
  fi
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o orderer.blackcreek.tech:7050 -C $CHANNEL_NAME -n ${CHAINCODE_NAME} $PEER_CONN_PARMS -c '{"function":"getSaveEstateByDp","Args":["f363b02c-e1f1-42d4-9078-b4848c45fb42"]}' >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode invoke -o orderer.blackcreek.tech:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CHAINCODE_NAME} $PEER_CONN_PARMS -c '{"function":"getSaveEstateByDp","Args":["f363b02c-e1f1-42d4-9078-b4848c45fb42"]}' >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
