...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.blackcreek.tech | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.blackcreek.tech
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile BlackCreekChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  set -x
  configtxgen -profile BlackCreekChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/DCMSPanchors.tx -channelID $CHANNEL_NAME -asOrg DCMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for DCMSP..."
    exit 1
  fi
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for DPMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile BlackCreekChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/DPMSPanchors.tx -channelID $CHANNEL_NAME -asOrg DPMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
